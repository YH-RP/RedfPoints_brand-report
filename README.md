# RedfPoints_brand-report
<!DOCTYPE html>
<html lang="zh-Hant">
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>Red Points 2026 監控報告 (V9 完善版)</title>
    <script src="https://cdn.jsdelivr.net/npm/chart.js"></script>
    <link href="https://fonts.googleapis.com/css2?family=Noto+Sans+TC:wght@300;400;500;700&display=swap" rel="stylesheet">
    <link href="https://fonts.googleapis.com/css2?family=Roboto:wght@500;700&display=swap" rel="stylesheet">
    
    <style>
        :root {
            --primary: #c0565b;       
            --primary-bg: #fff1f2; 
            --secondary: #475569;     
            --bg: #f8fafc;            
            --card-bg: #ffffff;
            --text-main: #334155;
            --text-light: #94a3b8;
            --chart-colors: #e76f51, #2a9d8f, #e9c46a, #264653, #f4a261;
        }

        body { font-family: 'Noto Sans TC', sans-serif; background: var(--bg); color: var(--text-main); margin: 0; padding: 40px 20px; }
        .container { max-width: 1200px; margin: 0 auto; }

        /* Header (修改：標題變粗) */
        .header-section { display: flex; justify-content: space-between; align-items: flex-end; margin-bottom: 30px; padding-bottom: 20px; border-bottom: 1px solid #e2e8f0; }
        .header-title h1 { margin: 0; font-size: 28px; font-weight: 700; color: var(--secondary); letter-spacing: 1px; } /* font-weight 改為 700 */
        .header-title span { font-size: 14px; color: var(--text-light); margin-top: 5px; display: block;}
        
        .upload-area { display: flex; gap: 10px; }
        .style-select { padding: 8px; border: 1px solid #cbd5e1; border-radius: 8px; color: var(--secondary); }
        .btn-upload { background: var(--primary); color: white; padding: 8px 20px; border-radius: 50px; cursor: pointer; border: none; box-shadow: 0 4px 10px rgba(192, 86, 91, 0.2); transition: 0.3s; }
        .btn-upload:hover { transform: translateY(-2px); box-shadow: 0 6px 15px rgba(192, 86, 91, 0.3); }
        #csvInput { display: none; }

        /* Sections */
        .section-box { margin-bottom: 40px; }
        .section-header-row { display: flex; justify-content: space-between; align-items: center; margin-bottom: 15px; }
        .section-title { font-size: 18px; color: var(--secondary); font-weight: 700; display: flex; align-items: center; gap: 8px; }
        .section-title::before { content: ''; width: 4px; height: 18px; background: var(--primary); border-radius: 2px; }
        .month-selector { padding: 5px 15px; border-radius: 20px; border: 1px solid var(--primary); color: var(--primary); background: white; cursor: pointer; outline: none; font-weight: bold; }

        /* Insight Card (Summary) */
        .insight-card {
            background: linear-gradient(135deg, #fff 0%, #fffafa 100%);
            border-left: 5px solid var(--primary); border-radius: 12px; padding: 20px 25px; margin-bottom: 30px;
            box-shadow: 0 4px 20px rgba(0,0,0,0.03); border: 1px solid #f1f5f9;
        }
        .insight-title { font-size: 15px; font-weight: 700; color: var(--primary); margin-bottom: 10px; display: flex; align-items: center; gap: 5px; }
        .insight-content { font-size: 15px; line-height: 1.8; color: var(--secondary); }
        .highlight-num { font-weight: 700; color: #e76f51; background: #fff1f2; padding: 0 4px; border-radius: 4px; }
        .highlight-text { font-weight: 700; color: #264653; }

        /* KPI Grid */
        .kpi-grid { display: grid; grid-template-columns: repeat(5, 1fr); gap: 15px; }
        .kpi-card { background: white; padding: 20px; border-radius: 12px; text-align: center; box-shadow: 0 4px 6px rgba(0,0,0,0.02); transition: 0.3s; border: 1px solid #f1f5f9; }
        .kpi-card:hover { transform: translateY(-3px); box-shadow: 0 10px 15px rgba(0,0,0,0.05); }
        .kpi-label { font-size: 13px; color: var(--text-light); margin-bottom: 5px; }
        .kpi-num { font-size: 28px; font-weight: 700; color: var(--primary); font-family: 'Roboto', sans-serif; }
        .kpi-unit { font-size: 12px; color: #cbd5e1; margin-top: 5px; }

        /* 新增：KPI 註解樣式 */
        .kpi-footer-note {
            text-align: right;
            font-size: 12px;
            color: #94a3b8;
            margin-top: 8px;
            font-weight: 400;
        }

        /* Assets Table & Chart */
        .assets-container { background: white; border-radius: 16px; padding: 25px; box-shadow: 0 4px 6px rgba(0,0,0,0.02); border: 1px solid #f1f5f9; }
        .chart-area { height: 300px; margin-bottom: 20px; }
        .custom-table { width: 100%; border-collapse: collapse; font-size: 14px; }
        .custom-table th { text-align: left; padding: 10px; color: #64748b; border-bottom: 2px solid #f1f5f9; }
        .custom-table td { padding: 10px; border-bottom: 1px solid #f8fafc; color: #334155; }
        .custom-table tr:hover td { background: #fdf2f2; }
        .ytd-col { font-weight: bold; color: var(--primary); background: #fffafa; }

        /* Bottom Charts */
        .split-layout { display: grid; grid-template-columns: 1fr 1fr; gap: 20px; }
        .chart-card { background: white; border-radius: 16px; padding: 20px; height: 300px; box-shadow: 0 4px 6px rgba(0,0,0,0.02); border: 1px solid #f1f5f9; }

        @media (max-width: 900px) {
            .kpi-grid { grid-template-columns: 1fr 1fr; }
            .split-layout { grid-template-columns: 1fr; }
            .header-section { flex-direction: column; align-items: flex-start; gap: 15px; }
        }
    </style>
</head>
<body>

<div class="container">
    <div class="header-section">
        <div class="header-title">
            <h1>Red Points Report</h1>
            <span id="statusText">請匯入 2026 年度 CSV 資料</span>
        </div>
        <div class="upload-area">
            <select id="encodingSelect" class="style-select">
                <option value="Big5">Big5 (Excel預設)</option>
                <option value="UTF-8">UTF-8</option>
            </select>
            <label for="csvInput" class="btn-upload">＋ 匯入檔案</label>
            <input type="file" id="csvInput" accept=".csv" />
        </div>
    </div>

    <div class="section-box">
        <div class="section-header-row">
            <div class="section-title">1. 關鍵指標 (KPI)</div>
            <select id="monthSelect" class="month-selector" style="display:none;"></select>
        </div>

        <div class="kpi-grid">
            <div class="kpi-card">
                <div class="kpi-label">偵測數量</div>
                <div class="kpi-num" id="kpi-detect">-</div>
                <div class="kpi-unit">當月件數</div>
            </div>
            <div class="kpi-card">
                <div class="kpi-label">成功下架</div>
                <div class="kpi-num" id="kpi-remove">-</div>
                <div class="kpi-unit">當月件數</div>
            </div>
            <div class="kpi-card">
                <div class="kpi-label">不可執行</div>
                <div class="kpi-num" id="kpi-unable">-</div>
                <div class="kpi-unit">當月件數</div>
            </div>
            <div class="kpi-card">
                <div class="kpi-label">執行成功率</div>
                <div class="kpi-num" id="kpi-rate">-</div>
                <div class="kpi-unit" style="color:#2a9d8f">本月效率</div>
            </div>
            <div class="kpi-card">
                <div class="kpi-label">經濟影響 (萬)</div>
                <div class="kpi-num" id="kpi-impact">-</div>
                <div class="kpi-unit">挽回損失</div>
            </div>
        </div>
        <div class="kpi-footer-note">
            *註：經濟影響計算公式為 Price × Stock（由 Red Points 提供）
        </div>
    </div>

    <div id="insightSection" class="section-box" style="display:none;">
        <div class="insight-card">
            <div class="insight-title">💡 <span id="insightMonthTitle">本月觀察重點</span></div>
            <div class="insight-content" id="insightContent">計算中...</div>
        </div>
    </div>

    <div class="section-box">
        <div class="section-title" style="margin-bottom:20px;">2. 侵權資產分析 (趨勢與明細)</div>
        <div class="assets-container">
            <div class="chart-area"><canvas id="assetsChart"></canvas></div>
            <div style="overflow-x:auto;">
                <table class="custom-table" id="assetTable">
                    <thead><tr id="assetTableHeader"><th>載入中...</th></tr></thead>
                    <tbody id="assetTableBody"></tbody>
                </table>
            </div>
        </div>
    </div>

    <div class="split-layout">
        <div class="section-box" style="margin:0;">
            <div class="section-title" style="margin-bottom:20px;">3. 保護類型分佈</div>
            <div class="chart-card"><canvas id="typeChart"></canvas></div>
        </div>
        <div class="section-box" style="margin:0;">
            <div class="section-title" style="margin-bottom:20px;">4. 高風險網域排行</div>
            <div class="chart-card"><canvas id="domainChart"></canvas></div>
        </div>
    </div>
</div>

<script>
    // --- Config ---
    Chart.defaults.font.family = "'Noto Sans TC', sans-serif";
    Chart.defaults.color = '#94a3b8';
    Chart.defaults.scale.grid.color = '#f1f5f9';
    const pastelColors = ['#e76f51', '#2a9d8f', '#e9c46a', '#264653', '#f4a261', '#8ab17d'];
    const months = ['1月','2月','3月','4月','5月','6月','7月','8月','9月','10月','11月','12月'];
    let globalData = null;
    let charts = {};

    // --- Events ---
    document.getElementById('csvInput').addEventListener('change', function(e) {
        const file = e.target.files[0];
        if(!file) return;
        const reader = new FileReader();
        reader.onload = function(e) {
            try {
                const data = parseCSV(e.target.result);
                updateDashboard(data, file.name);
            } catch(err) { alert("讀取錯誤: " + err); }
        };
        reader.readAsText(file, document.getElementById('encodingSelect').value);
        e.target.value = '';
    });

    document.getElementById('monthSelect').addEventListener('change', (e) => {
        if(globalData) {
            const idx = parseInt(e.target.value);
            renderKPI(globalData, idx);
            generateInsights(globalData, idx);
        }
    });

    // --- Functions ---
    function parseCSV(csvText) {
        const lines = csvText.split(/\r\n|\n/);
        let section = null;
        let data = { detected:[], removed:[], unable:[], rate:[], impact:[], assets:{}, types:{}, domains:[] };

        lines.forEach(line => {
            if(!line.trim()) return;
            const cols = line.split(',');
            if(line.includes("指標項目")) { section = "KPI"; return; }
            if(line.includes("侵權資產")) { section = "Assets"; return; }
            if(line.includes("保護類型")) { section = "Types"; return; }
            if(line.includes("網域活動")) { section = "Domains"; return; }

            const getVals = () => cols.slice(1, 13).map(v => {
                let n = parseFloat(v); return isNaN(n) ? null : n;
            });

            if(section === "KPI") {
                const vals = getVals();
                if(line.includes("偵測數量")) data.detected = vals;
                if(line.includes("成功下架")) data.removed = vals;
                if(line.includes("不可執行")) data.unable = vals;
                if(line.includes("執行成功率")) data.rate = vals;
                if(line.includes("經濟影響")) data.impact = vals;
            }
            if(section === "Assets" && !line.includes("合計") && cols[0].length > 2) {
                const vals = getVals();
                if(vals.some(v => v !== null)) data.assets[cols[0]] = vals;
            }
            if(section === "Types" && (line.includes("Counterfeit") || line.includes("Brand abuse"))) {
                data.types[cols[0]] = getVals(); 
            }
            if(section === "Domains" && cols[1] && cols[1].includes("(")) {
                const m = cols[1].match(/(.*) \((\d+)\)/);
                if(m) data.domains.push({ name: m[1].trim(), val: parseInt(m[2]) });
            }
        });
        return data;
    }

    function updateDashboard(data, filename) {
        globalData = data;
        let indices = [];
        data.detected.forEach((v, i) => { if(v !== null) indices.push(i); });
        const lastIdx = indices[indices.length - 1] || 0;

        document.getElementById('statusText').textContent = `來源: ${filename} (至 ${months[lastIdx]})`;
        
        const sel = document.getElementById('monthSelect');
        sel.innerHTML = '';
        indices.forEach(i => {
            let opt = document.createElement('option');
            opt.value = i; opt.text = months[i];
            sel.appendChild(opt);
        });
        sel.value = lastIdx;
        sel.style.display = 'block';

        renderKPI(data, lastIdx);
        generateInsights(data, lastIdx);
        renderAssetTable(data, lastIdx);
        renderCharts(data, lastIdx);
    }

    function generateInsights(data, idx) {
        const div = document.getElementById('insightSection');
        div.style.display = 'block';
        document.getElementById('insightMonthTitle').textContent = `【本月觀察重點 - ${months[idx]}】`;

        let assetList = [];
        for(let [name, vals] of Object.entries(data.assets)) {
            assetList.push({ name: name, val: vals[idx] || 0 });
        }
        assetList.sort((a, b) => b.val - a.val);

        const top1 = assetList[0] || { name: '無', val: 0 };
        const top2 = assetList[1] || { name: '無', val: 0 };

        let countCounterfeit = (data.types['Counterfeit'] && data.types['Counterfeit'][idx]) || 0;
        let countAbuse = (data.types['Brand abuse'] && data.types['Brand abuse'][idx]) || 0;
        let totalType = countCounterfeit + countAbuse;
        let percentCounterfeit = totalType > 0 ? ((countCounterfeit / totalType) * 100).toFixed(1) : 0;
        
        const html = `
            <span class="highlight-text">"${top1.name}"</span> 與 <span class="highlight-text">"${top2.name}"</span> 
            仍為主要侵權對象，分別達 <span class="highlight-num">${top1.val}</span> 件 與 <span class="highlight-num">${top2.val}</span> 件。<br>
            假貨 (Counterfeit) 仍佔絕大多數 <span class="highlight-num">${countCounterfeit}</span> 件 
            (≈ <span class="highlight-num">${percentCounterfeit}%</span>)，
            品牌濫用 (Brand abuse) 數量極少 (僅 <span class="highlight-num">${countAbuse}</span> 件)。
        `;
        document.getElementById('insightContent').innerHTML = html;
    }

    function renderKPI(data, idx) {
        document.getElementById('kpi-detect').textContent = data.detected[idx] || 0;
        document.getElementById('kpi-remove').textContent = data.removed[idx] || 0;
        document.getElementById('kpi-unable').textContent = data.unable[idx] || 0;
        let r = data.rate[idx];
        document.getElementById('kpi-rate').textContent = (r <= 1 ? (r*100).toFixed(1) : r) + "%";
        document.getElementById('kpi-impact').textContent = data.impact[idx] || 0;
    }

    function renderAssetTable(data, lastIdx) {
        const thead = document.getElementById('assetTableHeader');
        const tbody = document.getElementById('assetTableBody');
        let headerHTML = `<th>資產名稱</th>`;
        for(let i=0; i<=lastIdx; i++) headerHTML += `<th>${months[i]}</th>`;
        headerHTML += `<th class="ytd-col">YTD 總計</th>`;
        thead.innerHTML = headerHTML;

        let bodyHTML = '';
        for(let [name, vals] of Object.entries(data.assets)) {
            let rowTotal = 0;
            let rowHTML = `<td>${name}</td>`;
            for(let i=0; i<=lastIdx; i++) {
                let v = vals[i] || 0;
                rowTotal += v;
                rowHTML += `<td>${v}</td>`;
            }
            rowHTML += `<td class="ytd-col">${rowTotal}</td>`;
            bodyHTML += `<tr>${rowHTML}</tr>`;
        }
        tbody.innerHTML = bodyHTML;
    }

    function renderCharts(data, lastIdx) {
        if(charts.assets) charts.assets.destroy();
        let assetDatasets = [];
        let c = 0;
        for(let [k, v] of Object.entries(data.assets)) {
            assetDatasets.push({
                label: k, data: v.slice(0, lastIdx+1),
                backgroundColor: pastelColors[c++ % pastelColors.length], borderRadius: 4
            });
        }
        charts.assets = new Chart(document.getElementById('assetsChart'), {
            type: 'bar',
            data: { labels: months.slice(0, lastIdx+1), datasets: assetDatasets },
            options: { responsive: true, maintainAspectRatio: false, scales: { x:{stacked:true, grid:{display:false}}, y:{stacked:true} } }
        });

        if(charts.types) charts.types.destroy();
        let typeLabels = Object.keys(data.types);
        let typeData = typeLabels.map(key => (data.types[key][lastIdx] || 0));
        charts.types = new Chart(document.getElementById('typeChart'), {
            type: 'doughnut',
            data: {
                labels: typeLabels,
                datasets: [{ 
                    data: typeData, 
                    backgroundColor: [pastelColors[0], '#cbd5e1'], borderWidth: 0
                }]
            },
            options: { responsive: true, maintainAspectRatio: false, cutout: '70%' }
        });

        if(charts.domain) charts.domain.destroy();
        charts.domain = new Chart(document.getElementById('domainChart'), {
            type: 'bar',
            data: {
                labels: data.domains.map(d => d.name),
                datasets: [{
                    data: data.domains.map(d => d.val),
                    backgroundColor: pastelColors[3], borderRadius: 6, barThickness: 20
                }]
            },
            options: { indexAxis: 'y', responsive: true, maintainAspectRatio: false, plugins: { legend: { display: false } }, scales: { x:{grid:{display:false}}, y:{grid:{display:false}} } }
        });
    }
</script>
</body>
</html>
